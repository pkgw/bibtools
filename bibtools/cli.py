...
class Btmerge(multitool.Command):
    ...
    summary = (
        "Make BibTeX entries needed for an .aux file, based on an existing BibTeX file."
    )
...
class CanonJournal(multitool.Command):
    ...
    def invoke(self, args, app=None, **kwargs):
        ...
        for pub in app.db.pub_query("refdata NOT NULL"):
            ...
            app.db.execute(
                "UPDATE pubs SET refdata = ? WHERE id == ?", (json.dumps(rd), pub.id)
            )
...
class DumpCrossref(multitool.Command):
    ...
    def invoke(self, args, app=None, **kwargs):
        ...
        if kind == "doi":
            ...
            if pub.doi is None:
                die(
                    'publication "%s" has no associated DOI, which is necessary', idtext
                )
            doi = pub.doi
        ...
        def invoke(self, args, app=None, **kwargs):
            ...
            try:
                for pub in app.locate_pubs(args[1:], autolearn=True):
                    app.db.execute(
                        "INSERT OR IGNORE INTO publists VALUES (?, "
            ...
            try:
                for idtext in args[1:]:
                    ...
                    for pub in app.locate_pubs((idtext,)):
                        c.execute(
                            "DELETE FROM publists WHERE name == ? AND pubid == ?",
                            (dbgroupname, pub.id),
                        )
                        ndeleted += c.rowcount
...
class Info(multitool.Command):
    ...
    def invoke(self, args, app=None, **kwargs):
        ...
        bold, bred, red, reset = get_color_codes(
            None, "bold", "bold-red", "red", "reset"
        )
...
class Recent(multitool.Command):
    ...
    def invoke(self, args, app=None, **kwargs):
        ...
        pubs = app.db.pub_fquery(
            ...
            "WHERE p.id == h.pubid ORDER BY date DESC LIMIT ?",
            n,
        )
...
class Rmpush(multitool.Command):
    ...
    def invoke(self, args, app=None, **kwargs):
        ...
        for idtext in args:
            ...
            if sha1 is None:
                die(
                    "no saved PDF for %s, and cannot figure out how to download it",
                    idtext,
                )
